...
test("Flapper#handleKick: Creates the entity Auction.",
  () => {
    ...
    let event = changetype<Kick>(tests.helpers.events.getNewEvent([
    ...
    assert.fieldEquals("Auction", id.toString()+"-0", "quantity", lot.toString())
    assert.fieldEquals("Auction", id.toString()+"-0", "bidAmount", bid.toString())
    assert.fieldEquals("Auction", id.toString()+"-0", "highestBidder", event.transaction.from.toHexString())
    assert.fieldEquals("Auction", id.toString()+"-0", "endTime", "2500")
    clearStore()
  }
)