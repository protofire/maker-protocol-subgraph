...
test("Vat#handleSlip modifies a user's collateral balance via adding wad", () => {
  ...
  assert.fieldEquals('Collateral', usr + '-' + ilk, 'updatedAt', event.block.timestamp.toString())
  assert.fieldEquals('Collateral', usr + '-' + ilk, 'updatedAtBlock', event.block.number.toString())
  assert.fieldEquals('Collateral', usr + '-' + ilk, 'updatedAtTransaction', event.transaction.hash.toHexString())
